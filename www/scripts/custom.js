...
window.showNotification = function (message, type) {
    ...
    toast.innerHTML = '<i class="fas ' + icon + '"></i><span>' + message + '</span><button class="toast-close" aria-label="Close">&times;</button>';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    var pwaName = "MissVIP"; //Local Storage Names for PWA
    ...
    function init_template() {
        ...
        //show only if login 
        var snama = localStorage.getItem('nama');

        if (snama == '' || snama == null) {
            $('.login-only').hide(); // sembunyikan semua yg belum berhak diakses
            snama = 'Guest';
        } else {
            $('.login-only').show();
            $('.nologin').hide();
        ...
        //         document.querySelectorAll('.greeting-text')[0].insertAdjacentHTML('beforeend', 'Hello, ' + 'Gorgeous');
        //     }
        ...
        if (splide.length) {
            ...
            setTimeout(slide2, 500);

            function slide2() {
                var singleSlider2 = document.querySelectorAll('.single-slider2');
                if (singleSlider2.length) {
                    singleSlider2.forEach(function(e) {
                        var single = new Splide('#' + e.id, {
                            type: 'loop',
                            autoplay: true,
                            interval: 5000,
            ...
            var singleSlider3 = document.querySelectorAll('.single-slider3');
            if (singleSlider3.length) {
                singleSlider3.forEach(function(e) {
                    var single = new Splide('#' + e.id, {
                        type: 'loop',
                        autoplay: true,
                        interval: 6000,
        ...
        */
        ...
        //Reading Time
        var proBelanja = document.querySelectorAll('#pro-belanja-month');
        if (proBelanja.length) {
            loadMyProfile();
        }
        ...
        if (qr_image.length) {
            var qr_this = localStorage.getItem('handphone');
            ...
            if (qr_btn) {
                qr_btn.addEventListener('click', function() {
                    ...
                    var qr_api_address = 'https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=';
                    var qr_img = '<img class="mx-auto polaroid-effect shadow-l mt-4 delete-qr" width="200" src="' + qr_api_address + lochandphone + '" alt="img"><p class="font-11 text-center mb-0">' + get_qr_url + '</p>'
        ...
        if (isPWA === true) {
            ...
            if (!isStandalone) {
                var PWA_DISMISS_KEY = pwaName + '-PWA-Prompt';
                var PWA_TIME_KEY = pwaName + '-PWA-Timeout-Value';
                var remindMs = pwaRemind * 24 * 60 * 60 * 1000;
                var now = Date.now();
                var lastTime = localStorage.getItem(PWA_TIME_KEY);

                if (lastTime && (now - Number(lastTime)) > remindMs) {
                    localStorage.removeItem(PWA_DISMISS_KEY);
                    localStorage.setItem(PWA_TIME_KEY, now);
                }
                if (!lastTime) { localStorage.setItem(PWA_TIME_KEY, now); }

                if (localStorage.getItem(PWA_DISMISS_KEY) !== 'dismissed') {
                    ...
                    function showPWABanner() {
                        ...
                        banner.innerHTML = [
                            '<img src="app/icons/icon-72x72.png" width="40" height="40" style="border-radius:8px;flex-shrink:0">',
                            '<div style="flex:1">',
                            '  <div style="font-weight:600;font-size:13px">Install Solusi POS</div>',
                            '  <div style="font-size:12px;opacity:.85">Tambahkan ke home screen untuk akses lebih cepat</div>',
                            '</div>',
                            '<button id="pwa-install-btn" style="background:#fff;color:#1a73e8;border:none;border-radius:8px;padding:7px 14px;font-weight:700;font-size:13px;cursor:pointer;flex-shrink:0">Install</button>',
                            '<button id="pwa-dismiss-btn" style="background:transparent;color:#fff;border:none;font-size:20px;cursor:pointer;flex-shrink:0;line-height:1;padding:0 4px">&times;</button>'
                        ].join('');
                        document.body.appendChild(banner);

                        document.getElementById('pwa-dismiss-btn').addEventListener('click', function () {
                            banner.remove();
                            localStorage.setItem(PWA_DISMISS_KEY, 'dismissed');
                            localStorage.setItem(PWA_TIME_KEY, Date.now());
                        });
                        ...
                        if (deferredPrompt) {
                            ...
                            installBtn.addEventListener('click', function () {
                                alert('Di Safari: tap tombol Share lalu pilih "Add to Home Screen".');
                            });
                        }
                    }
                    ...
                    if (isIOS) { setTimeout(showPWABanner, 3500); }
                }
            }
